...
const Featured = () => {
  const {featuredItems} = useContext(StoreContext)
    ...
    <div className='featured' id='featured'>
      ...
      <div className="featured-list">
        {featuredItems.map((item, index)=>{
            return (
                <Listing className="listing" key={item.id} id={item.id} name={item.name} image={item.image} new_price={item.new_price} old_price={item.old_price}></Listing>
            )
        })}
      </div>
      <hr />
